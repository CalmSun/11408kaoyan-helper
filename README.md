...
| 📺 哔哩哔哩在线学习视频（v3.5.3 ~ v3.5.7） | 学习资料页「哔哩哔哩」模式：扫码 / Cookie 两种登录方式（凭证本地持久化）；我的收藏夹浏览与分页加载；关键词视频搜索（自动领取 buvid 防风控）；个性化推荐与热门视频推荐 6×4 网格布局、换一批真刷新 / 加载更多（热门首次访问自动刷新）；视频点赞 / 投币 / 收藏一键三连（实时状态回显）；原生播放器弹窗屏幕居中、分 P 切换、DASH 高清晰度播放（MSE 音视频分离 + 标准 RFC 6381 MIME 组装 + SourceBuffer 配额自动清理，大会员支持 1080P 高码率 / 4K，含 durl 回退）、备用 CDN 自动切换；UP 主卡片（紧凑尺寸）、投稿视频分页播放与相关视频置于播放卡片右侧栏独立滚动；弹幕显示与一键开关（行内样式 + 全局 keyframes 注入渲染，滚动 / 顶部 / 底部弹幕，偏好持久化）；播放流经主进程回环代理并注入 Referer 绕过防盗链，与本地资料功能互不影响 |
